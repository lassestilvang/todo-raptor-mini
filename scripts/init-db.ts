...
export async function initDb(options?: { dbPath?: string; migrationsDir?: string }) {
  const dbPath = options?.dbPath || process.env.DATABASE_URL || path.join(process.cwd(), 'db', 'data.db')
  const migrationsDir = options?.migrationsDir || path.join(process.cwd(), 'db', 'migrations')

  if (!fs.existsSync(path.dirname(dbPath))) fs.mkdirSync(path.dirname(dbPath), { recursive: true })

  const isBun = typeof process !== 'undefined' && (process as any).versions && (process as any).versions.bun
...
if (require.main === module) {
  initDb()
    ...
    .catch((e) => {
      console.error(e)
      console.error('\nOptions:')
      console.error("  - run 'bun install' again to allow prebuilt better-sqlite3 binaries to download")
