...
export async function logActivity(entityType: string, entityId: string, action: string, payload: any = null, performedBy: string | null = null) {
  ...
  if (conn) {
    ...
    conn.exec(`INSERT INTO activity_log (id, entity_type, entity_id, action, payload, performed_by, created_at) VALUES ('${id}', '${entityType}', '${entityId}', '${action}', ${payloadSql}, ${performedBySql}, '${now}')`)
    return row
  }
...
export async function getActivityForEntity(entityType: string, entityId: string) {
  ...
  try {
    ...
    return rows.map((r: any) => ({ id: r.id, action: r.action, payload: r.payload ? JSON.parse(r.payload) : null, createdAt: r.created_at }))
  } catch (e) {
    // Fallback to raw SQL for SQL.js driver
    // @ts-ignore - runtime global
    const conn: any = (globalThis as any).__SQL_JS_CONN__ || null
    if (!conn) throw e
    const res = conn.exec(`SELECT id, action, payload, created_at FROM activity_log WHERE entity_type = '${entityType}' AND entity_id = '${entityId}'`)
    const mapped = (res && res[0] && res[0].values) ? res[0].values.map((v: any[]) => ({ id: v[0], action: v[1], payload: v[2] ? JSON.parse(v[2]) : null, createdAt: v[3] })) : []
    return mapped
  }
}
