...
export async function GET(req: Request) {
  ...
  try {
    ...
    let rows = []
    try {
      rows = await _db.select().from(attachmentSchema).where(attachmentSchema.task_id.eq(taskId)).all()
    } catch (_err) {
    ...
    if ((!rows || rows.length === 0) && conn) {
      const res = conn.exec(`SELECT id, task_id, filename, size, mime, storage_key, created_at FROM attachments WHERE task_id = '${taskId}'`)
      const mapped = (res && res[0] && res[0].values) ? res[0].values.map((v: any[]) => ({ id: v[0], task_id: v[1], filename: v[2], size: v[3], mime: v[4], storage_key: v[5], created_at: v[6] })) : []
...
export async function POST(req: Request) {
  ...
  try {
    const _db = await import('../../../lib/db').then(m => m.getDb())
    if (_db) {
      const insertQuery: any = _db.insert((await import('../../../db/schema')).attachments).values(row)
