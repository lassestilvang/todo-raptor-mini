  ...
  return (
    <nav className="flex flex-col gap-3">
        ...
        <ul className="mt-2">
          {views.map((v) => (
            <li key={v.key} className="mt-2">
              <Link href={v.href} className="text-sm hover:underline">
                {v.label}
              </Link>
            </li>
          ))}
      ...
      <div className="mt-4">
        ...
        <ul className="mt-2">
          <li className="mt-2">
            <Link href="/app/lists/inbox" className="text-sm">
              📥 Inbox{' '}
              {overdue ? (
                <span className="ml-2 text-xs bg-red-600 rounded-full px-2">{overdue}</span>
              ) : null}
            </Link>
          </li>
          {/* dynamic lists */}
