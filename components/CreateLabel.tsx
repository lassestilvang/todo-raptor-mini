...
export default function CreateLabel() {
  ...
  async function onSubmit(data: any) {
    await fetch('/api/labels', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) })
    reset()
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex gap-2 mt-2">
      <input {...register('icon')} placeholder="Icon" className="w-12 px-2 py-1 rounded bg-slate-800" />
      <input {...register('name')} placeholder="Label" className="flex-1 px-2 py-1 rounded bg-slate-800" />
